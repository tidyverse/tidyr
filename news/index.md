...
- [`fill()`](https://tidyr.tidyverse.org/reference/fill.md) gains a
...
- [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md) produces a
  more helpful error message when columns cannot be cast to `ptype`
  ([@mgirlich](https://github.com/mgirlich),
  [\#1477](https://github.com/tidyverse/tidyr/issues/1477)).

- [`unite()`](https://tidyr.tidyverse.org/reference/unite.md) no longer
  errors if you provide a selection that doesn’t select any columns.
  Instead, it returns a column containing the empty string
  ([\#1548](https://github.com/tidyverse/tidyr/issues/1548),
  [@catalamarti](https://github.com/catalamarti)).

- [`pivot_wider_spec()`](https://tidyr.tidyverse.org/reference/pivot_wider_spec.md)
...
- New family of consistent string separating functions:
  ...
  [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) and
...
- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) gains a
...
- The `...` argument of both
  ...
  [`pivot_longer_spec()`](https://tidyr.tidyverse.org/reference/pivot_longer_spec.md),
  [`pivot_wider_spec()`](https://tidyr.tidyverse.org/reference/pivot_wider_spec.md),
  [`build_longer_spec()`](https://tidyr.tidyverse.org/reference/pivot_longer_spec.md),
  and
  [`build_wider_spec()`](https://tidyr.tidyverse.org/reference/pivot_wider_spec.md)
...
- All functions deprecated in tidyr 1.0 and 1.2 (the old lazyeval
  ...
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md)) now
...
- [`uncount()`](https://tidyr.tidyverse.org/reference/uncount.md) is now
  generic so implementations can be provided for objects other than data
  frames ([@mgirlich](https://github.com/mgirlich),
  [\#1358](https://github.com/tidyverse/tidyr/issues/1358)).

- [`uncount()`](https://tidyr.tidyverse.org/reference/uncount.md) gains
  a `...` argument. It comes between the required and the optional
  arguments ([@mgirlich](https://github.com/mgirlich),
  [\#1358](https://github.com/tidyverse/tidyr/issues/1358)).

- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md),
  [`complete()`](https://tidyr.tidyverse.org/reference/complete.md),
  [`expand()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`fill()`](https://tidyr.tidyverse.org/reference/fill.md) now document
  their support for grouped data frames created by
  [`dplyr::group_by()`](https://dplyr.tidyverse.org/reference/group_by.html)
...
- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md),
  [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md),
  [`unnest_longer()`](https://tidyr.tidyverse.org/reference/unnest_longer.md),
  and
  [`unnest_wider()`](https://tidyr.tidyverse.org/reference/unnest_wider.md)
...
- [`pack()`](https://tidyr.tidyverse.org/reference/pack.md),
  [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md),
  [`chop()`](https://tidyr.tidyverse.org/reference/chop.md), and
  [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md) all gain
  an `error_call` argument, which in turn improves some of the error
  calls shown in
  [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) and various
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) adjacent
  functions ([\#1446](https://github.com/tidyverse/tidyr/issues/1446)).

- [`chop()`](https://tidyr.tidyverse.org/reference/chop.md),
  [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md), and
  [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md) all gain
  `...`, which must be empty
  ([\#1447](https://github.com/tidyverse/tidyr/issues/1447)).

- [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md) does a
  ...
  [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md), like
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) and
  [`unnest_wider()`](https://tidyr.tidyverse.org/reference/unnest_wider.md)
...
- [`fill()`](https://tidyr.tidyverse.org/reference/fill.md) now works
  correctly when there is a column named `.direction` in `data`
...
- [`complete()`](https://tidyr.tidyverse.org/reference/complete.md) and
  [`expand()`](https://tidyr.tidyverse.org/reference/expand.md) no
  longer allow you to complete or expand on a grouping column. This was
  never well-defined since completion/expansion on a grouped data frame
  happens “within” each group and otherwise has the potential to produce
  erroneous results
...
- [`pivot_longer()`](https://tidyr.tidyverse.org/reference/pivot_longer.md)’s
...
- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) and
  [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md)’s `ptype`
  argument now accepts a single empty ptype which will be applied to all
  `cols` ([\#1284](https://github.com/tidyverse/tidyr/issues/1284)).

- [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md) now
  silently skips over any non-data frame columns specified by `cols`.
  This matches the existing behavior of
  [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md) and
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md)
...
- [`unnest_longer()`](https://tidyr.tidyverse.org/reference/unnest_longer.md)’s
  `indices_to` and `values_to` arguments now accept a glue
  specification, which is useful when unnesting multiple columns.

- For [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md),
  [`unnest_longer()`](https://tidyr.tidyverse.org/reference/unnest_longer.md),
  and
  [`unnest_wider()`](https://tidyr.tidyverse.org/reference/unnest_wider.md),
...
- [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md),
  [`unnest_longer()`](https://tidyr.tidyverse.org/reference/unnest_longer.md),
  and
  [`unnest_wider()`](https://tidyr.tidyverse.org/reference/unnest_wider.md)’s
...
- [`drop_na()`](https://tidyr.tidyverse.org/reference/drop_na.md),
  [`replace_na()`](https://tidyr.tidyverse.org/reference/replace_na.md),
  and [`fill()`](https://tidyr.tidyverse.org/reference/fill.md) have
...
- [`drop_na()`](https://tidyr.tidyverse.org/reference/drop_na.md) no
...
- [`pivot_wider()`](https://tidyr.tidyverse.org/reference/pivot_wider.md)’s
...
- [`pivot_longer()`](https://tidyr.tidyverse.org/reference/pivot_longer.md)’s
  ...
  (i.e. [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md))
  ([\#1233](https://github.com/tidyverse/tidyr/issues/1233)).

- [`check_pivot_spec()`](https://tidyr.tidyverse.org/reference/check_pivot_spec.md)
...
- The [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) generic
  now avoids computing on `.data`, making it more compatible with lazy
  tibbles ([\#1134](https://github.com/tidyverse/tidyr/issues/1134)).

- The `.names_sep` argument of the data.frame method for
  [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) is now
  actually used
  ([\#1174](https://github.com/tidyverse/tidyr/issues/1174)).

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md)’s
  `ptype` argument now works as expected
  ([\#1158](https://github.com/tidyverse/tidyr/issues/1158)).

- [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md) no longer
  drops empty columns specified through `cols`
  ([\#1191](https://github.com/tidyverse/tidyr/issues/1191)).

- [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md) now works
  correctly with data frame columns containing 1 row but 0 columns
  ([\#1189](https://github.com/tidyverse/tidyr/issues/1189)).

- [`chop()`](https://tidyr.tidyverse.org/reference/chop.md) now works
  correctly with data frames with 0 rows
  ([\#1206](https://github.com/tidyverse/tidyr/issues/1206)).

- [`chop()`](https://tidyr.tidyverse.org/reference/chop.md)’s `cols`
...
- [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md) now
  respects `ptype` when unnesting a non-list column
  ([\#1211](https://github.com/tidyverse/tidyr/issues/1211)).

#### Rectangling

- [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md) no longer
  accidentally removes elements that have duplicated names
...
- The grouped data frame methods for
  [`complete()`](https://tidyr.tidyverse.org/reference/complete.md) and
  [`expand()`](https://tidyr.tidyverse.org/reference/expand.md) now move
  the group columns to the front of the result (in addition to the
  columns you completed on or expanded, which were already moved to the
  front). This should make more intuitive sense, as you are completing
  or expanding “within” each group, so the group columns should be the
  first thing you see
  ([\#1289](https://github.com/tidyverse/tidyr/issues/1289)).

- [`complete()`](https://tidyr.tidyverse.org/reference/complete.md) now
  applies `fill` even when no columns to complete are specified
  ([\#1272](https://github.com/tidyverse/tidyr/issues/1272)).

- [`expand()`](https://tidyr.tidyverse.org/reference/expand.md),
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md) now
  correctly retain `NA` values of factors
  ([\#1275](https://github.com/tidyverse/tidyr/issues/1275)).

- [`expand_grid()`](https://tidyr.tidyverse.org/reference/expand_grid.md),
  [`expand()`](https://tidyr.tidyverse.org/reference/expand.md),
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md) now
  silently apply name repair to automatically named inputs. This avoids
  a number of issues resulting from duplicate truncated names
...
- [`expand_grid()`](https://tidyr.tidyverse.org/reference/expand_grid.md),
  ...
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md) now
  allow columns from unnamed data frames to be used in expressions after
...
- [`expand_grid()`](https://tidyr.tidyverse.org/reference/expand_grid.md),
  ...
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md) now
  work correctly with data frames containing 0 columns but \>0 rows
  ([\#1189](https://github.com/tidyverse/tidyr/issues/1189)).
...
- [`fill()`](https://tidyr.tidyverse.org/reference/fill.md) now treats
  `NaN` like any other missing value
...
- [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md) is now
...
- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) is now
  much faster ([@mgirlich](https://github.com/mgirlich),
  [@DavisVaughan](https://github.com/DavisVaughan),
  [\#1127](https://github.com/tidyverse/tidyr/issues/1127)).

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) no
  ...
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) that has
  now become an error.
...
- [`uncount()`](https://tidyr.tidyverse.org/reference/uncount.md) error
  for non-integer weights and gives a clearer error message for negative
  weights ([@mgirlich](https://github.com/mgirlich),
...
- [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md) now
...
- [`unnest_longer()`](https://tidyr.tidyverse.org/reference/unnest_longer.md),
  [`unnest_wider()`](https://tidyr.tidyverse.org/reference/unnest_wider.md),
  and [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md) do a
  better job simplifying list-cols. They no longer add unneeded
...
- [`pack()`](https://tidyr.tidyverse.org/reference/pack.md) and
  [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) gains a
  `.names_sep` argument allows you to strip outer names from inner
  names, in symmetrical way to how the same argument to
  [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md) and
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) combines
...
- [`unnest_wider()`](https://tidyr.tidyverse.org/reference/unnest_wider.md)
  ...
  [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) and with
  [`pivot_wider()`](https://tidyr.tidyverse.org/reference/pivot_wider.md),
...
- [`chop()`](https://tidyr.tidyverse.org/reference/chop.md) now creates
  list-columns of class
  ...
  empty, allowing
  [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md) to
  reconstitute a data frame with the correct number and types of column
  even when there are no observations.

- [`drop_na()`](https://tidyr.tidyverse.org/reference/drop_na.md) now
  preserves attributes of unclassed vectors
  ([\#905](https://github.com/tidyverse/tidyr/issues/905)).

- [`expand()`](https://tidyr.tidyverse.org/reference/expand.md),
  [`expand_grid()`](https://tidyr.tidyverse.org/reference/expand_grid.md),
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md) once
  again evaluate their inputs iteratively, so you can refer to freshly
  created columns, e.g. `crossing(x = seq(-2, 2), y = x)`
  ([\#820](https://github.com/tidyverse/tidyr/issues/820)).

- [`expand()`](https://tidyr.tidyverse.org/reference/expand.md),
  [`expand_grid()`](https://tidyr.tidyverse.org/reference/expand_grid.md),
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md) gain a
  `.name_repair` giving you control over their name repair strategy
  ([@jeffreypullin](https://github.com/jeffreypullin),
  [\#798](https://github.com/tidyverse/tidyr/issues/798)).

- [`extract()`](https://tidyr.tidyverse.org/reference/extract.md) lets
  you use `NA` in `into`, as documented
  ([\#793](https://github.com/tidyverse/tidyr/issues/793)).
...
- [`pack()`](https://tidyr.tidyverse.org/reference/pack.md)’s first
...
- [`unnest_auto()`](https://tidyr.tidyverse.org/reference/unnest_auto.md)
...
See
[`vignette("in-packages")`](https://tidyr.tidyverse.org/articles/in-packages.md)
for a detailed transition guide.

- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) and
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) have new
  syntax. The majority of existing usage should be automatically
...
- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) now
...
- The first argument of
  [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) has changed
  from `data` to `.data`.

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) uses the
  [emerging tidyverse
...
- [`unnest_()`](https://tidyr.tidyverse.org/reference/deprecated-se.md)/[`nest_()`](https://tidyr.tidyverse.org/reference/deprecated-se.md)
  and the lazyeval methods for
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md)/[`nest()`](https://tidyr.tidyverse.org/reference/nest.md)
...
- [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md) and
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md) now
  return 0-row outputs if any input is a length-0 vector. If you want to
  preserve the previous behaviour which silently dropped these inputs,
  you should convert empty vectors to `NULL`. (More discussion on this
  general pattern at
...
[`spread()`](https://tidyr.tidyverse.org/reference/spread.md) and
[`gather()`](https://tidyr.tidyverse.org/reference/gather.md). They have
been carefully redesigned to be easier to learn and remember, and
include many new features. Learn more in
[`vignette("pivot")`](https://tidyr.tidyverse.org/articles/pivot.md).

These functions resolve multiple existing issues with
[`spread()`](https://tidyr.tidyverse.org/reference/spread.md)/[`gather()`](https://tidyr.tidyverse.org/reference/gather.md).
...
[`unnest_wider()`](https://tidyr.tidyverse.org/reference/unnest_wider.md),
[`unnest_auto()`](https://tidyr.tidyverse.org/reference/unnest_auto.md),
and [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md). They
...
New [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md) provides
a convenient way of plucking components of a list-column out into their
own top-level columns
...
[`nest()`](https://tidyr.tidyverse.org/reference/nest.md) and
[`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) have been
updated with new interfaces that are more closely aligned to evolving
tidyverse conventions. They use the theory developed in
...
[`vignette("nest")`](https://tidyr.tidyverse.org/articles/nest.md),
...
[`nest()`](https://tidyr.tidyverse.org/reference/nest.md) and
[`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) will
...
[`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) has been
overhauled:
...
Under the hood,
[`nest()`](https://tidyr.tidyverse.org/reference/nest.md) and
[`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) are
implemented with
[`chop()`](https://tidyr.tidyverse.org/reference/chop.md),
[`pack()`](https://tidyr.tidyverse.org/reference/pack.md),
[`unchop()`](https://tidyr.tidyverse.org/reference/chop.md), and
...
- [`pack()`](https://tidyr.tidyverse.org/reference/pack.md) and
  [`unpack()`](https://tidyr.tidyverse.org/reference/pack.md) allow you
  to pack and unpack columns into data frame columns
  ([\#523](https://github.com/tidyverse/tidyr/issues/523)).

- [`chop()`](https://tidyr.tidyverse.org/reference/chop.md) and
  [`unchop()`](https://tidyr.tidyverse.org/reference/chop.md) chop up
  rows into sets of list-columns.
...
- [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md),
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`expand()`](https://tidyr.tidyverse.org/reference/expand.md) have
  been rewritten to use the vctrs package. This should not affect much
  existing code, but considerably simplifies the implementation and
...
- [`full_seq()`](https://tidyr.tidyverse.org/reference/full_seq.md) now
  also works when gaps between observations are shorter than the given
  `period`, but are within the tolerance given by `tol`. Previously,
  gaps between consecutive observations had to be in the range
  \[`period`, `period + tol`\]; gaps can now be in the range
...
- [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md) now
  takes the unique values of data frame inputs, not just vector inputs
  ([\#490](https://github.com/tidyverse/tidyr/issues/490)).

- [`gather()`](https://tidyr.tidyverse.org/reference/gather.md) throws
  an error if a column is a data frame
  ([\#553](https://github.com/tidyverse/tidyr/issues/553)).

- [`extract()`](https://tidyr.tidyverse.org/reference/extract.md) (and
  hence
  [`pivot_longer()`](https://tidyr.tidyverse.org/reference/pivot_longer.md))
  can extract multiple input values into a single output column
  ([\#619](https://github.com/tidyverse/tidyr/issues/619)).

- [`fill()`](https://tidyr.tidyverse.org/reference/fill.md) is now
  implemented using
...
- [`fill()`](https://tidyr.tidyverse.org/reference/fill.md) now accepts
  `downup` and `updown` as fill directions
  ([@coolbutuseless](https://github.com/coolbutuseless),
  [\#505](https://github.com/tidyverse/tidyr/issues/505)).

- [`unite()`](https://tidyr.tidyverse.org/reference/unite.md) gains
...
- [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md)
  ...
  [`expand()`](https://tidyr.tidyverse.org/reference/expand.md) which is
  built on top of
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md))

- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) is
  compatible with dplyr 0.8.0.

- [`spread()`](https://tidyr.tidyverse.org/reference/spread.md) works
  when the id variable has names
...
- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) will run
  with named and unnamed list-columns of same length
...
- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) now
  accepts `NA` as a column name in the `into` argument to denote columns
  which are omitted from the result.
...
- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) weakens
  test of “atomicity” to restore previous behaviour when unnesting
  factors and dates
...
- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) now
  correctly uses -1 to refer to the far right position, instead of -2.
  If you depended on this behaviour, you’ll need to switch on
...
- `complete(data)` now returns `data` rather than throwing an error
  ...
  [`complete()`](https://tidyr.tidyverse.org/reference/complete.md) with
  zero-length completions returns original input
...
- [`expand()`](https://tidyr.tidyverse.org/reference/expand.md) with
...
- [`expand()`](https://tidyr.tidyverse.org/reference/expand.md),
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`complete()`](https://tidyr.tidyverse.org/reference/complete.md) now
  complete empty factors instead of dropping them
  ([\#270](https://github.com/tidyverse/tidyr/issues/270),
  [\#285](https://github.com/tidyverse/tidyr/issues/285))

- [`extract()`](https://tidyr.tidyverse.org/reference/extract.md) has a
  better error message if `regex` does not contain the expected number
  of groups ([\#313](https://github.com/tidyverse/tidyr/issues/313)).
...
- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) on a
...
- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) no
  longer checks that `...` is empty, allowing methods to make use of it.
  This check was added in tidyr 0.4.0 (2016-02-02) to deprecate previous
  behaviour where `...` was passed to
  [`strsplit()`](https://rdrr.io/r/base/strsplit.html).

- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) and
  [`extract()`](https://tidyr.tidyverse.org/reference/extract.md) now
  insert columns in correct position when `drop = TRUE`
  ([\#394](https://github.com/tidyverse/tidyr/issues/394)).

- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) now
  works correctly counts from RHS when using negative integer `sep`
  values ([@markdly](https://github.com/markdly),
  [\#315](https://github.com/tidyverse/tidyr/issues/315)).

- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) gets
  improved warning message when pieces aren’t as expected
...
- [`spread()`](https://tidyr.tidyverse.org/reference/spread.md) now
...
- [`spread()`](https://tidyr.tidyverse.org/reference/spread.md) no
...
- [`unite()`](https://tidyr.tidyverse.org/reference/unite.md) now works
  (as documented) if you don’t supply any variables
  ([\#355](https://github.com/tidyverse/tidyr/issues/355)).

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) gains
  `preserve` argument which allows you to preserve list columns without
  unnesting them
  ([\#328](https://github.com/tidyverse/tidyr/issues/328)).

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) can
...
- The SE variants
  [`gather_()`](https://tidyr.tidyverse.org/reference/deprecated-se.md),
  [`spread_()`](https://tidyr.tidyverse.org/reference/deprecated-se.md)
  and
  [`nest_()`](https://tidyr.tidyverse.org/reference/deprecated-se.md)
...
- If you see error messages about objects or functions not found, it is
  ...
  [`gather()`](https://tidyr.tidyverse.org/reference/gather.md) and its
  `...` argument. This change makes the code more robust by disallowing
  ambiguous scoping. Consider the following code:
...
([`gather()`](https://tidyr.tidyverse.org/reference/gather.md) being a
prime example). This is not tidy because the symbol do not represent any
actual object. Our workaround is to capture these arguments using
...
[`drop_na()`](https://tidyr.tidyverse.org/reference/drop_na.md)) as well
as single variables (the `col` argument of
[`extract()`](https://tidyr.tidyverse.org/reference/extract.md) and
[`separate()`](https://tidyr.tidyverse.org/reference/separate.md), and
the `key` and `value` arguments of
[`spread()`](https://tidyr.tidyverse.org/reference/spread.md)). This
...
- Changed deprecation message of
  [`extract_numeric()`](https://tidyr.tidyverse.org/reference/extract_numeric.md)
  to point to
...
- [`expand()`](https://tidyr.tidyverse.org/reference/expand.md),
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md), and
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md) now
  silently drop zero-length inputs.

- [`crossing_()`](https://tidyr.tidyverse.org/reference/deprecated-se.md)
  and
  [`nesting_()`](https://tidyr.tidyverse.org/reference/deprecated-se.md)
  are versions of
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md) and
  [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md) that
...
- [`gather()`](https://tidyr.tidyverse.org/reference/gather.md) can now
  gather together list columns
...
- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) returns
...
- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) and
  [`unite()`](https://tidyr.tidyverse.org/reference/unite.md) now
  automatically drop removed variables from grouping
  ([\#159](https://github.com/tidyverse/tidyr/issues/159),
  [\#177](https://github.com/tidyverse/tidyr/issues/177)).

- [`spread()`](https://tidyr.tidyverse.org/reference/spread.md) gains a
  `sep` argument. If not-null, this will name columns as “keyvalue”.
  Additionally, if sep is `NULL` missing values will be converted to
  `<NA>` ([\#68](https://github.com/tidyverse/tidyr/issues/68)).

- [`spread()`](https://tidyr.tidyverse.org/reference/spread.md) works in
  the presence of list-columns
  ([\#199](https://github.com/tidyverse/tidyr/issues/199))

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) works
  with non-syntactic names
  ([\#190](https://github.com/tidyverse/tidyr/issues/190)).

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) gains a
  `sep` argument. If non-null, this will rename the columns of nested
  data frames to include both the original column name, and the nested
  column name, separated by `.sep`
...
- Fixed bug in [`nest()`](https://tidyr.tidyverse.org/reference/nest.md)
  where nested data was ending up in the wrong row
...
[`nest()`](https://tidyr.tidyverse.org/reference/nest.md) and
[`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) have been
overhauled to support a useful way of structuring data frames: the
...
- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) now produces
  a single list of data frames called “data” rather than a list column
  for each variable. Nesting variables are not included in nested data
  frames. It also works with grouped data frames made by
  [`dplyr::group_by()`](https://dplyr.tidyverse.org/reference/group_by.html).
  You can override the default column name with `.key`.

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) gains a
  `.drop` argument which controls what happens to other list columns. By
  default, they’re kept if the output doesn’t require row duplication;
  otherwise they’re dropped.

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) now has
  [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html)
...
- [`expand()`](https://tidyr.tidyverse.org/reference/expand.md) once
...
- [`nesting()`](https://tidyr.tidyverse.org/reference/expand.md) and
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md) allow
  you to create nested and crossed data frames from individual vectors.
  [`crossing()`](https://tidyr.tidyverse.org/reference/expand.md) is
  similar to
...
- [`fill()`](https://tidyr.tidyverse.org/reference/fill.md) fills in
...
- [`gather()`](https://tidyr.tidyverse.org/reference/gather.md) now
...
- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) now
  warns if defunct … argument is used
...
- New [`complete()`](https://tidyr.tidyverse.org/reference/complete.md)
...
- [`nest()`](https://tidyr.tidyverse.org/reference/nest.md) is the
  complement of
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md)
  ([\#3](https://github.com/tidyverse/tidyr/issues/3)).

- [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) can now
  work with multiple list-columns at the same time. If you don’t supply
  any columns names, it will unlist all list-columns
  ([\#44](https://github.com/tidyverse/tidyr/issues/44)).
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) can also
  handle columns that are lists of data frames
...
- [`expand()`](https://tidyr.tidyverse.org/reference/expand.md) now
...
- [`extract()`](https://tidyr.tidyverse.org/reference/extract.md) is 10x
  faster because it now uses stringi instead of base R regular
...
- The internals of
  [`spread()`](https://tidyr.tidyverse.org/reference/spread.md) have
...
- [`spread()`](https://tidyr.tidyverse.org/reference/spread.md) gives a
  more informative error message if `key` or `value` don’t exist in the
  input data ([\#36](https://github.com/tidyverse/tidyr/issues/36)).

- [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) only
  displays the first 20 failures
  ...
  [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) no
  longer throws an error if the number of pieces aren’t as expected -
  instead it uses drops extra values and fills on the right and gives a
  warning.

- If the input is NA
  [`separate()`](https://tidyr.tidyverse.org/reference/separate.md) and
  [`extract()`](https://tidyr.tidyverse.org/reference/extract.md) both
  return silently return NA outputs, rather than throwing an error.
  ([\#77](https://github.com/tidyverse/tidyr/issues/77))

- Experimental
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) method
  for lists has been removed.
...
- Experimental
  [`expand()`](https://tidyr.tidyverse.org/reference/expand.md) function
  ([\#21](https://github.com/tidyverse/tidyr/issues/21)).

- Experiment
  [`unnest()`](https://tidyr.tidyverse.org/reference/unnest.md) function
  for converting named lists into data frames.
  ([\#3](https://github.com/tidyverse/tidyr/issues/3),
...
- [`gather()`](https://tidyr.tidyverse.org/reference/gather.md) has
  better defaults if `key` and `value` are not supplied. If `...` is
  omitted, [`gather()`](https://tidyr.tidyverse.org/reference/gather.md)
  selects all columns
...
- [`spread()`](https://tidyr.tidyverse.org/reference/spread.md) gains
  `drop` argument, which allows you to preserve missing factor levels
  ([\#25](https://github.com/tidyverse/tidyr/issues/25)). It converts
  factor value variables to character vectors, instead of embedding a
  matrix inside the data frame
