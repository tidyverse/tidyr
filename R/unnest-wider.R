...
col_to_wide <- function(col, name, strict, names_sep, call = caller_env()) {
  if (is.null(col)) {
  ...
  elt_ptype <- elt_to_wide(col_ptype, name = name, strict = strict, names_sep = names_sep, call = call)
  elt_ptype <- vec_ptype(elt_ptype)

  # Avoid expensive dispatch from `[[.list_of`
  out <- tidyr_new_list(col)
  out <- map(out, elt_to_wide, name = name, strict = strict, names_sep = names_sep, call = call)
...
elt_to_wide <- function(x, name, strict, names_sep, call = caller_env()) {
  if (is.null(x)) {
    x <- list()
  }
  ...
  if (is.null(names_sep)) {
    names(x) <- vec_as_names(names2(x), repair = "unique", call = call)
  } else {
    outer <- name
    ...
    if (is.null(inner)) {
      ...
      inner <- vec_as_names(inner, repair = "unique", call = call)
    }
