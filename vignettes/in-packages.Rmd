...
* It describes migration patterns for the transition from tidyr v0.8.3 to 
  ...
  in order to increase consistency within tidyr.
...
Compared to dplyr and ggplot2, most tidyr functions have __select semantics__, not the more common mutate semantics. This means that you typically provide tidyr functions with an expression that works with column names (like `starts_with("x")` or `a:c`) not with column values (like `y = x * 2`), so the way that you use them within a package wrap is a little different.

There are three main cases that you'll encounter:

* The column names are fixed: use `one_of("col1", "col2", "col3")`.

* The columns names are in a character vector: use `one_of(cols)`.

* The user specify the column names using select syntax: use `{{ cols }}`.

### Fixed column names

If you know the names of the columns, the code works in the same way in and out of a package:
...
If the column names are in a character vector (possibly from a function call), you can use `one_of()`, as above:
...
To provide an interface that works like the tidyr function that you're wrapping, you'll can pass the argument along using `{{ arg }}`. `{{ }}` is a special tidy eval operator that captures the expression supplied by the user and forwards it to another tidy eval-enabled function.
...
If you decide to support tidyselect syntax, I'd recomend also re-exporting the select helpers so that users can use (e.g.) `starts_with()` without any extra work. There's no helper to [do this yet](https://github.com/r-lib/usethis/issues/868), but you can copy-and-paste the code that [tidyr uses](https://github.com/tidyverse/tidyr/blob/00a66b44d2f304a73e953dd73139db61ed9cc289/R/tidyr.R#L29-L53).
...
If you use travis already, we **strongly** recommend adding a build that tests with the development version of tidyr; see above for details.

This section briefly describes how to run different code for different version of tidyr, then goes through the major changes that might require workarounds: 

* `nest()` and `unnest()` get new interfaces.
* `nest()` preserves groups.
* `nest_()` and `unnest_()` are defunct.

If you're struggling with an problem that's not described here, please reach out via [github](https://github.com/tidyverse/tidyr/issues/new) or [email](hadley@rstudio.com) so we can help out.
...
We highly recommend keeping this as a function because it provides an obvious place to jot any transition notes for your package, and it makes it easier to remove transitional code later on.
...
If your new code uses a function that only exists in tidyr 1.0.0, you will get an `R CMD check` `NOTE`: this is one of the few notes that you can explain in your CRAN submission comments. Just mention that it's for forward compatibility with tidyr 1.0.0, and CRAN will let your package through.
...
*   The use of `...` to make data is a problematic pattern we're moving away from.
...
If you need a quick and dirty fix with having to think, just call `nest_legacy()` instead of `nest()`. It's the same as `nest()` in v0.8.3:
...
* `.id` has been duplicated because it's easily replicated outside of unnest.
...
If you are pressed for time, and need a quick fix, call `unnest_legacy()` instead of `unnest()`. It's the same as `unnest()` in v0.8.3:
...
* We are gradually remove support for the old lazyeval framework in favour of 
  the more powerful tidy eval.
