...
Note that I don't believe it makes sense to describe a dataset as being in "long form". Length is a relative term, and you can only say (e.g.) that dataset A is longer than dataset B. 
...
This records the billboard rank of songs over time. It is very similar in form to the `pew` data above, but the data encoded in the column names is really a number, not a string. 
...
We can use `names_prefix` to automatically strip off the `wk` prefix, and `col_types` to specify that `week` should be an integer.
...
billboard %>% 
  pivot_longer(
    ...
    values_to = "rank",
    values_drop_na = TRUE,
    col_ptypes = list(week = integer()),
  )
```

This has one small problem: the `week` variable is a character variable rather than an integer. 
...
`country`, `iso2`, `iso3`, and `year` are already variables, so can be left as is. We want to pivot the columns from `new_sp_m014` to `newrel_f65`. These columns encode four variables in their names:
...
We can extract these variables using a regular expression:
...
So far, we have been working with data frames that have a single value column. But many important pivotting problems involve multiple values. In this section, you'll learn how to pivot this sort of data. The following example is adapted from the [data.table vignette](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-reshape.html), which inspired tidyr's solution to this problem:
...
Note that we have two pieces of information about each child: their gender and their date of birth. These need to go into separate value columns in the result. We tell `pivot_longer()` to do this by suppling a character vector to `names_to` and a regular expresion to `names_sep`:
...
Here `.value` is a special sentinel that represents the value column in the output, and overrides the value of the `value_to` argument. Note that I use `values_drop_na = TRUE` here because this shape of the data forces the creation of explicit missing variables for observations that don't exist. This ensures that family 2 has a single row in the output.
...
This dataset contains four pairs of variables (`x1` + `y1`, `x2` + `y2`, etc) that underlie Anscombe's quartet, a collection of four datasets that have the same summary statistics (mean, sd, correlation etc), but have quite different data. We want to produce a dataset with columns `graph`, `x` and `y`. Here we can't use a regular expression to split up the column names, and instead specify a split position:
...
```{r}
production %>% pivot_wider(names_from = c(product, country), values_from = value)
```
...
We could fix that after pivotting, but we can tackle the problem in another way: by generating a pivot spec. Using a pivoting spec is overkill for this problem, but it's a good bridge between very simple cases and the more complex cases you'll learn about later. A pivoting spec is a data frame that describes the metadata stored in the column name, with one row for each column, and one column for each variable mashed into the column name. 
...
This seems like a lot of work to get something very similar to the previous result, but as you'll see later, it generalises in useful ways.
...
```{r}
construction <- read_csv("construction.csv", col_types = list())
construction
```
...
production %>% pivot_wider(spec = spec) %>% head()
```

This works, but the column names could be improved. I think it would be better to have columns `rent`, `rent_moe`, `income`, and `income_moe`, which we can do with a manual spec. There are many ways that you could generate the spec, but the key is that we need to generate every combination of the `variable` values and `estimate`/`moe`, and then carefully generate the column name:
