...
To see how this works, lets return to the simplest case of pivotting applied to the `pew` dataset. Now pivotting happens in two steps: we first create a spec object (using `pivot_longer_spec()`) then use that to describe the pivotting operation:

```{r}
spec <- pew %>% pivot_longer_spec(
  cols = -religion, 
  names_to = "income",
  values_to = "count"
)
