...
[^readr]: It's not uncommon to use readr functions when generating pivoting specifications. This isn't too surprising as the job of the readr package is to turn text into data.
...
spec <- pivot_long_spec(pnl, c(starts_with("y"), starts_with("z"))) %>%
  separate(name, c(".value", "time"), 1, convert = TRUE)
spec

pnl %>% pivot_long(spec = spec)
```
