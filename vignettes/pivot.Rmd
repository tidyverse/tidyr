...
```{r}
spec <- pew %>% pivot_spec_long(-1, names_to = "count", values_to = "income")
spec
```
...
```{r}
billboard %>% pivot_spec_long(starts_with("wk"), names_to = "week", values_to = "rank")
```

```{r}
spec <- billboard %>%
  pivot_spec_long(starts_with("wk"), names_to = "week", values_to = "rank") %>%
  mutate(week = parse_number(week))
spec
```

```{r}
billboard %>% pivot_long(spec)
```
...
spec <- who %>%
  pivot_spec_long(new_sp_m014:newrel_f65, values_to = "count") %>%
...
```{r}
who %>% pivot_long(spec, na.rm = TRUE)
```
...
anscombe %>% 
  pivot_long(spec) %>% 
  pivot_wide(pivot_spec_wide(., var, value))
```
...
weather_spec <- weather %>%
  pivot_spec_long(d1:d31, names_to = "day", values_to = "temp") %>%
  mutate(day = parse_number(day))
weather_spec
weather2 <- pivot_long(weather, weather_spec, na.rm = TRUE)
weather2

spec2 <- weather2 %>% pivot_spec_wide(element, temp)
spec2
weather2 %>% pivot_wide(spec2)
```
...
spec <- pivot_spec_wide(df, names_from = type, values_from = meta)
df2 <- df %>% pivot_wide(spec)

spec2 <- pivot_spec_wide(df2, names_from = var, values_from = value)
df2 %>% select(-row) %>% pivot_wide(spec2)
```
