...
The tidy data frame explicitly tells us the definition of an observation. In this classroom, every combination of `name` and `assessment` is a single measured observation. The dataset also informs us of missing values, which might need to be adjusted later. If we assume that missing values indicate that a student was absent at assessment time, the reason for a student's absence could determine how their missing value is later dealth with (replaced with a zero for skipping class, or dropped if the student performs additional work).
