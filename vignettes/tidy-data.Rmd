...
library(stringr)
preg2 <-preg %>% 
  pivot_longer(cols = c(treatmenta:treatmentb), names_to = "treatment", values_to = "n") %>% 
...
Surprisingly, most messy datasets, including types of messiness not explicitly described above, can be tidied with a small set of tools: pivoting (using **pivot_longer** and **pivot_wider**) and separating. The following sections illustrate each problem with a real dataset that I have encountered, and show how to tidy them.
...
To tidy this dataset, we first use pivot_longer to gether up all the `wk` columns. The column names give the `week` and the values are the `rank`s:
...
Here we use `filter(!is.na())` to drop any missing values from the rank column. In this data, missing values represent weeks that the song wasn't in the charts, so can be safely dropped.
...
```{r}
tb2 <- tb %>% 
  pivot_longer(cols = c(-iso2, -year), names_to = "demo", values_to = "n") %>% 
  filter(!is.na(n)) %>% 
  arrange(demo, iso2, year)
tb2
```
...
This dataset is mostly tidy, but the `element` column is not a variable; it stores the names of variables. (Not shown in this example are the other meteorological variables `prcp` (precipitation) and `snow` (snowfall)). Fixing this requires the pivot_wider operation. This performs the inverse of pivot_longer by spreading the `element` and `value` columns back out into the columns:
