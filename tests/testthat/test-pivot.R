...
test_that("can drop missing values", {
  ...
  sp <- pivot_spec_long(df, x:y)
  pv <- pivot_long(df, sp, na.rm = TRUE)
...
test_that("can override default output column type", {
  df <- tibble(x = "x", y = 1)
  sp <- pivot_spec_long(df, x:y)
  pv <- pivot_long(df, sp, ptype = list(value = list()))
...
test_that("duplicated keys produce list column", {
  ...
  sp <- pivot_spec_wide(df, key, val)
  expect_warning(pv <- pivot_wide(df, sp), "list-col")
...
test_that("warn when overwriting existing column", {
  ...
  expect_message(pv <- pivot_wide(df, sp), "New names")
})
