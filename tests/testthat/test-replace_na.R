...
test_that("can only be length 0", {
  expect_snapshot(replace_na(1, 1:10), error = TRUE, cnd_class = TRUE)
})
...
test_that("replacement must be castable to `data`", {
  x <- c(1L, NA)
  expect_snapshot(replace_na(x, 1.5), error = TRUE, cnd_class = TRUE)
})
...
test_that("replacement must be castable to corresponding column", {
  df <- tibble(a = c(1L, NA))
  expect_snapshot(replace_na(df, list(a = 1.5)), error = TRUE, cnd_class = TRUE)
})
