...
test_that("crossing checks for bad inputs", {
  expect_snapshot(crossing(x = 1:10, y = quote(a)), error = TRUE, cnd_class = TRUE)
})
...
test_that("expand_grid() can control name_repair", {
  ...
  expect_snapshot(expand_grid(x = x, x = x), error = TRUE, cnd_class = TRUE)
...
test_that("grid_dots() reject non-vector input", {
  expect_snapshot(grid_dots(lm(1 ~ 1)),  error = TRUE, cnd_class = TRUE)
})
