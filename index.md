...
in
[`vignette("tidy-data")`](https://tidyr.tidyverse.org/articles/tidy-data.md).
...
- “Rectangling”, which turns deeply nested lists (as from JSON) into
  ...
  [`hoist()`](https://tidyr.tidyverse.org/reference/hoist.md), and
...
- Nesting converts grouped data to a form where each group becomes a
  ...
  [`vignette("nest")`](https://tidyr.tidyverse.org/articles/nest.md) for
  more details.
...
- Splitting and combining character columns. Use
  ...
  [`unite()`](https://tidyr.tidyverse.org/reference/unite.md) to combine
  multiple columns into a single character column.
...
- Make implicit missing values explicit with
  ...
  [`fill()`](https://tidyr.tidyverse.org/reference/fill.md), or a known
  value with
