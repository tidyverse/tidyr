...
- cols:
  ...
  \<[`tidy-select`](https://tidyr.tidyverse.org/reference/tidyr_tidy_select.md)\>
...
- names_sep, names_pattern:
  ...
  can either be a numeric vector (specifying positions to break on), or
  a single string (specifying a regular expression to split on).

  `names_pattern` takes the same specification as
  [`extract()`](https://tidyr.tidyverse.org/reference/extract.md), a
...
[`gather()`](https://tidyr.tidyverse.org/reference/gather.md), designed
to be both simpler to use and to handle more use cases. We recommend you
use `pivot_longer()` for new code;
[`gather()`](https://tidyr.tidyverse.org/reference/gather.md) isn't
